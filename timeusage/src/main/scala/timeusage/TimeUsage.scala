...
object TimeUsage {
  ...
  /** @return An RDD Row compatible with the schema produced by `dfSchema`
    ...
    */
  def row(line: List[String]): Row = Row.fromSeq(line.head.toString :: line.tail.map(_.toDouble))
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    ...
    val grouped = columnNames.groupBy{
      colName => prefixes.find{
        case (prefixList, _) => prefixList.exists(colName.startsWith)
      }.getOrElse(List.empty, -1)._2
    }.map{
      case (group, cols) => group -> cols.map(new Column(_))
    }
